...
ROOT=Path('/Users/grant/IdeaProjects/myopencart'); DATE='2026-07-30'; BASE='https://shoplovanest.com'
class P(HTMLParser):
 def __init__(self): super().__init__(); self.tags=[]; self.text=[]; self.title=''; self._in_title=False; self.metas=[]; self.links=[]; self.imgs=[]; self.h1=0; self.h2=0; self.h3=0
...
results=[]; banned=['minor','underage','teen','schoolgirl','pornographic','porn','xxx','medical grade','cure','treat infertility','guaranteed orgasm','therapy','therapeutic']
for slug in slugs:
 ...
 image_ok=bool(hero.get('alt') and hero.get('title') and any(m.get('property')=='og:image' for m in p.metas) and any(m.get('property')=='og:image:alt' for m in p.metas) and any(m.get('name')=='twitter:image' for m in p.metas) and any(m.get('name')=='twitter:image:alt' for m in p.metas) and 'ImageObject' in html)
 banned_hits=[b for b in banned if re.search(r'\b'+re.escape(b)+r'\b', text.lower())]
 exact_kw_ok=True
 art=next(a for a in state['articles'] if a['slug']==slug)
 for kw in art.get('primary_keywords',[]):
  if text.lower().count(kw.lower())>8: exact_kw_ok=False
 checks={'title_len':10<len(p.title.strip())<60,'meta_len':80<len(meta_content)<155,'h1_one':p.h1==1,'h2_structure':p.h2>=8,'gtag_once':html.count('G-P2LJRXN3D1')==2 and html.index('G-P2LJRXN3D1')<html.index('<meta charset'),'word_count':len(words)>=1000,'quick_answer':'Quick Answer' in text,'red_flags':'Red Flags / when to slow down before checkout' in text,'faq_schema':'FAQPage' in html and p.h3>=4,'authority_refs':len(refs)>=3 and all(u.startswith('https://') for u in refs),'internal_links':len(set(related))>=2,'product_links':len(set(prod))>=1,'image_seo':image_ok,'no_banned':not banned_hits,'topic_depth':all(x in text for x in ['Red Flags','FAQ','References and useful sources']) and ('checklist' in text.lower() or 'sizing' in text.lower()),'keyword_natural':exact_kw_ok}
 score=round(sum(checks.values())/len(checks)*100)
 results.append({'slug':slug,'title':p.title.strip(),'title_len':len(p.title.strip()),'meta_len':len(meta_content),'h1_count':p.h1,'h2_count':p.h2,'gtag_id_occurrences':html.count('G-P2LJRXN3D1'),'words':len(words),'authority_refs':refs,'related_blog_links':len(set(related)),'product_links':len(set(prod)),'image_seo':image_ok,'banned_hits':banned_hits,'checks':checks,'quality_score':score,'status':'pass' if score>=85 and all(checks.values()) else 'fail'})
...
for slug in slugs+['']:
 ...
 if node is not None: sitemap[loc]={'lastmod':(node.find('s:lastmod',ns).text if node.find('s:lastmod',ns) is not None else None),'image_count':len(node.findall('image:image',ns)),'image_titles':[im.find('image:title',ns).text for im in node.findall('image:image',ns) if im.find('image:title',ns) is not None]}
...
(ROOT/'output/daily_blog_validation_2026-07-30.json').write_text(json.dumps(report,indent=2),encoding='utf-8')
