...
ROOT=Path('/Users/grant/IdeaProjects/myopencart'); DATE='2026-07-20'; BASE='https://shoplovanest.com'
...
for slug in slugs:
    ...
    art=state['articles'][slugs.index(slug)]
    c={
      ...
      'natural_readability_markers': all(x in text for x in ['If','Look for','Slow down']) and max(p_lens or [0])<125,
      'topic_specific_depth': sum(1 for term in art['primary_keywords'] if term.split()[0].lower() in text.lower())>=3,
