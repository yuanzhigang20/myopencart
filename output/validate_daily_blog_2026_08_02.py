...
ROOT=Path('/Users/grant/IdeaProjects/myopencart'); DATE='2026-08-02'; BASE='https://shoplovanest.com'
slugs=['silicone-anal-lube-safety-guide','vibrating-kegel-balls-safety-guide']
results=[]
...
for slug in slugs:
 ...
 img_meta=all(['property="og:image"' in html, 'property="og:image:alt"' in html, 'name="twitter:image"' in html, 'name="twitter:image:alt"' in html, ' alt="' in hero_tag and ' title="' in hero_tag, 'ImageObject' in html])
 related_count=len(re.findall(r'<div class="related">.*?</div>',html,re.S) and re.findall(r'<div class="related">.*?</div>',html,re.S)[0].split('<a href="/blog/'))-1
 prod_count=len(re.findall(r'<li><a href="/index.php',html))
 refs_count=len(re.findall(r'<section class="content-card refs">.*?</section>',html,re.S) and re.findall(r'<section class="content-card refs">.*?</section>',html,re.S)[0].split('href="http'))-1
...
(ROOT/'output/daily_blog_validation_2026-08-02.json').write_text(json.dumps(out,indent=2),encoding='utf-8')
