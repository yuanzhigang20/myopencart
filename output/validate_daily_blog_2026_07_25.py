...
import re,json
ROOT=Path('/Users/grant/IdeaProjects/myopencart'); DATE='2026-07-25'; BASE='https://shoplovanest.com'
...
for slug in slugs:
 ...
 product=[u for u in re.findall(r'<a href="([^"]+)"',html) if 'route=product/search' in u or 'route=information/contact' in u or 'route=information/information' in u]
 imgm=re.search(r'<img class="hero"([^>]*)>',html,re.S); imgtag=imgm.group(0) if imgm else ''
 sentences=[s.strip() for s in re.split(r'[.!?]+', strip_tags(re.sub(r'<script.*?</script>',' ',html,flags=re.S))) if len(s.split())>3]
 avg_sentence=sum(len(s.split()) for s in sentences)/max(1,len(sentences))
 long_paras=[p for p in re.findall(r'<p[^>]*>(.*?)</p>',html,re.S) if len(strip_tags(p).split())>95]
 checks={'title_len':len(title),'title_under_60':len(title)<60,'meta_len':len(meta),'meta_under_155':len(meta)<155,'h1_count':len(h1),'one_h1':len(h1)==1,'gtag_id_count':html.count('G-P2LJRXN3D1'),'gtag_exactly_once_config':html.count("gtag('config', 'G-P2LJRXN3D1')")==1,'gtag_immediately_after_head':html.startswith('<!doctype html><html lang="en"><head><!-- Google tag'),'word_count':len(words),'word_count_1000_plus':len(words)>=1000,'quick_answer':any('Quick Answer'==x for x in h2),'red_flags':any('Red Flags' in x for x in h2),'faq_count':len(re.findall(r'<h3\b',html)),'faqpage_jsonld':'FAQPage' in html,'internal_blog_links':len(set(internal)),'product_support_links':len(set(product)),'authority_refs':len(refs),'has_authority':len(refs)>=4,'og_image':'property="og:image"' in html,'og_image_alt':'property="og:image:alt"' in html,'twitter_image':'name="twitter:image"' in html,'twitter_image_alt':'name="twitter:image:alt"' in html,'image_alt_title_figcaption':bool(imgtag and attr(imgtag,'alt') and attr(imgtag,'title') and '<figcaption>' in html),'jsonld_imageobject':'ImageObject' in html,'banned_scan_pass':not re.search(r'\b(minor|teen|porn|xxx|guaranteed orgasm|guaranteed performance|cure|treats?|fertility|therapy)\b', text, re.I),'quick_answer_near_top':html.find('Quick Answer') < html.find('<section class="content-card"') if 'Quick Answer' in html else False,'readability_avg_sentence_under_24':avg_sentence<24,'no_overlong_paragraphs':len(long_paras)==0}
 must=['title_under_60','meta_under_155','one_h1','gtag_exactly_once_config','gtag_immediately_after_head','word_count_1000_plus','quick_answer','red_flags','faqpage_jsonld','has_authority','og_image','og_image_alt','twitter_image','twitter_image_alt','image_alt_title_figcaption','jsonld_imageobject','banned_scan_pass','readability_avg_sentence_under_24','no_overlong_paragraphs']
...
for slug in slugs:
 url=f'{BASE}/blog/{slug}/'; chunk=sm[sm.find(url):sm.find(url)+2500] if url in sm else ''
...
expected=len(list((ROOT/'upload/blog').glob('*/index.html')))
index_checks={'links_new_articles':all(f'/blog/{s}/' in index for s in slugs),'blog_count_meta_updated':f'{expected} discreet' in index and f'"numberOfItems": {expected}' in index,'expected_count':expected}
status='pass' if all(r['checks']['pass'] for r in results) and all(v['url_present'] and v['lastmod'] and v['image_metadata'] for v in sitemap.values()) and index_checks['links_new_articles'] and index_checks['blog_count_meta_updated'] else 'fail'
...
print(json.dumps(state['content_quality_validation'],indent=2)[:14000])
