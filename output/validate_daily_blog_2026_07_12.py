...
ROOT=Path('/Users/grant/IdeaProjects/myopencart'); BLOG=ROOT/'upload/blog'; DATE='2026-07-12'
slugs=['adult-toy-subscription-box-guide','thrusting-adult-toy-buyer-guide']
...
for slug in slugs:
    ...
    checks={
      ...
      'authority_refs': len(re.findall(r'rel="nofollow noopener"',html))>=4 and any(x in html for x in ['consumer.ftc.gov','www.ftc.gov']) and any(x in html for x in ['cpsc.gov','www.iec.ch','tsa.gov','uspis.gov']),
      ...
      'natural_readability_markers': all(x in html for x in ['Look for','Slow down','Pause if']) and max((len(p.split()) for p in re.findall(r'<p>(.*?)</p>',html,re.S)), default=0) < 130,
      'topic_specific_depth': (('subscription' in html.lower() and 'cancellation' in html.lower() and 'billing descriptor' in html.lower()) if slug.startswith('adult-toy-subscription') else ('motion' in html.lower() and 'motor' in html.lower() and 'charging' in html.lower() and 'water-resistance' in html.lower())),
